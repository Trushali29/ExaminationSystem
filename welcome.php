...
<body>
    ...
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
     ...
     <!-- THIS SCRIPT IS TO DISPLAY TIMER PROVIDE BY THE DATA -->
    <script>
    ...
    <!-- THIS SCRIPT WILL FETCH THE SELECTED ANSWER OF THE USER,SUBJECT,STUDENT NAME -->
    <script type="text/javascript">
        $(document).on('click','#pagination_button_submit',function(){
            var student_Exam_answer = new Array();
            var exam_subject = "<?php echo $exam_subject ?>";
            var student_name = "<?php echo $_SESSION['fullname'];?>";
            student_Exam_answer.push(radio_value);
            $.ajax({
                url: 'StudentResultDetails.php',
                data:{student_Exam_answer:student_Exam_answer,exam_subject:exam_subject,student_name:student_name},
                type:'post',
                success:function(data,status){
                    /* REDIRECT TO THE Main PAGE WHICH IN TURN IT WILL REDIRECT TO THE USER WHO HAS LOGGED IN */
                    $('#RedirectMainPage').modal('toggle');
                    const myTimeout = setTimeout(main_page, 4000);
                    function main_page() {
